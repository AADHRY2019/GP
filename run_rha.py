...
from model.rha import RHA
from model.rvqa import Randomvqa
...
def train(epoch, dataset, config, log_dir):
    ...
    with tf.Graph().as_default():
        model = RHA(model_config)
        model.build_inference()
        model.build_loss(train_config['reg_coeff'], train_config['shu_coeff'])
        #model.build_train(train_config['learning_rate'])
        model.build_train(1e-4)
        # rand_model = Randomvqa(model_config)
        # rand_model.build_inference()
        # rand_model.build_loss(train_config['reg_coeff'])
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            while dataset.has_train_batch:
                ...
                # _,loss_rand,acc_rand = sess.run(
                #     [rand_model.prediction,rand_model.loss,rand_model.acc],feed_dict
                # )
                total_loss += loss
                total_acc += acc
                if batch_idx % 100 == 0:
                    ...
                    # print('[Random Guess] epoch {}, batch {}/{}, loss {:.5f}, acc {:.5f}.'.format(
                    #     epoch, batch_idx, batch_total, loss_rand, acc_rand))
                batch_idx += 1
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            while dataset.has_val_example:
                ...
                # random_prediction = sess.run(rand_model.prediction,feed_dict=feed_dict)
                prediction = prediction[0]
                # random_prediction = random_prediction[0]
                if answerset[prediction] == answer:
                    correct += 1
                # if answerset[random_prediction] == answer:
                #     rand_correct +=1

            acc = correct / dataset.val_example_total
            print('\n[VAL] epoch {}, acc {:.5f}.\n'.format(epoch, acc))
            # rand_acc = rand_correct / dataset.val_example_total
            # # print('\n[VAL] epoch {}, rand_acc {:.5f}.\n'.format(epoch, rand_acc))
            summary = tf.Summary()
...
def predict(dataset,config,id) :
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=sess_config) as sess:
            try:
                vgg, c3d, question, answer, question_raw = dataset.get_test_example(id)
                feed_dict = {
                    model.appear: [vgg],
                    model.motion: [c3d],
                    model.question_encode: [question],
                }
                prediction, channel_weight = sess.run(
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            while dataset.has_test_example:
                try :
                    ...
                    prediction,  channel_weight = sess.run(
                        [model.prediction, model.channel_weight], feed_dict=feed_dict)
                    prediction = prediction[0]
...
#!python run_rha.py --mode test --gpu 0 --log log/gra --dataset msvd_qa --config 0
def main():
    """Main script."""
    ...
    if args.mode == 'train':
        ...
        predict(dataset,config,args.question)
